...
├── platforms/k8s/kubeadm/install.sh     # K8s kubeadm 入口
├── platforms/k8s/kind/install.sh        # Kind 入口
├── services/pgsql/backup/run.sh         # PG 备份入口
├── services/pgsql/restore/run.sh        # PG 恢复入口
├── services/pgsql/tests/test_pgsql.sh   # PG 单元测试（mock）
├── services/pgsql/tests/test_integration.sh # PG 集成测试（需真实 PG）
├── observability/prometheus/install.sh  # Prometheus 监控安装入口
└── docs/                                # 详细文档
```
...
- 新目录结构是当前主结构。
- `k8s/`、`kind/`、`pgsql/`、`prometheus/` 旧路径仍兼容保留，用于平滑迁移。
...
- 详细文档统一放在 `docs/` 目录下，并优先按领域分组，例如 `docs/platforms/k8s/`、`docs/services/pgsql/`
...
- Python 开发默认使用 `uv` 管理环境、依赖和测试。
- 项目 Python 版本以 `.python-version` 为准，当前固定为 `3.11`。
- 安装依赖优先使用 `uv sync --dev`，而不是手写多条 `pip install`。
- 运行命令优先使用 `uv run ...`，例如 `uv run bootstrap --help`、`uv run pytest tests/unit -v`。
- 若新增 Python 依赖，应同步更新 `pyproject.toml`，并优先保持 `uv` 工作流可用。
