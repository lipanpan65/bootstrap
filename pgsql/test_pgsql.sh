...
run_backup() {
    ...
    bash "$SCRIPT_DIR/backup.sh" "$@" > "$TMP_DIR/_last_output" 2>&1
...
run_restore() {
    ...
    AUTO_YES=true \
    _LIB_LOADED="" \
    bash "$SCRIPT_DIR/restore.sh" "$@" > "$TMP_DIR/_last_output" 2>&1
    echo $? > "$TMP_DIR/_last_rc"
    cat "$TMP_DIR/_last_output"
    return "$(cat "$TMP_DIR/_last_rc")"
}
...
test_backup_cleanup() {
    ...
    output=$(
        ...
        bash "$SCRIPT_DIR/backup.sh" -d cleandb --yes 2>&1
    ) || true
...
test_backup_types() {
    ...
    output=$(
        ...
        bash "$SCRIPT_DIR/backup.sh" -d manualdb --type manual --yes 2>&1
    ) || true
    ...
    output=$(
        ...
        bash "$SCRIPT_DIR/backup.sh" -d weeklydb --type weekly --yes 2>&1
    ) || true
...
test_restore_full() {
    ...
    _LIB_LOADED="" \
    PGDATABASE="" \
    bash "$SCRIPT_DIR/restore.sh" "$fake_dump" --yes > "$TMP_DIR/_last_output" 2>&1 || true
    output=$(cat "$TMP_DIR/_last_output")
...
test_backup_directory_format() {
    ...
    bash "$SCRIPT_DIR/backup.sh" -d testdb -F directory --yes > "$TMP_DIR/_last_output" 2>&1 || true
    local rc=$?
